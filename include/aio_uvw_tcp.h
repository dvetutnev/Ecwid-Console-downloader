...
private:
    ...
    void on_event(const Event& event) { publish(event); }
};
...
std::shared_ptr< TCPSocketWrapperSimple<AIO> > TCPSocketWrapperSimple<AIO>::create(std::shared_ptr<Loop> loop)
{
    ...
    {
        auto self = resource->shared_from_this();
        resource->tcp_handle->template on<ErrorEvent>( [self](const auto& err, const auto&) { self->on_event(err); } );
        resource->tcp_handle->template on<ConnectEvent>( [self](const auto& event, const auto&) { self->on_event(event); } );
        return resource;
    }
    return nullptr;
}
...
template< typename AIO >
void TCPSocketWrapperSimple<AIO>::read()
{

}

template< typename AIO >
void TCPSocketWrapperSimple<AIO>::stop()
{

}

template< typename AIO >
void TCPSocketWrapperSimple<AIO>::write()
{
